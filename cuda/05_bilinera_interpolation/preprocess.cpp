...
cv::Mat preprocess_gpu(cv::Mat& h_src, const int& tarH, const int& tarW,
                       Timer timer, int tactis) {
  uint8_t* d_tar = nullptr;
  uint8_t* d_src = nullptr;

  cv::Mat h_tar(cv::Size(tarW, tarH), CV_8UC3);
  int height = h_src.rows;
  int width = h_src.cols;
  int chan = 3;
  int src_size = height * width * chan;
  int tar_size = tarH * tarW * chan;
  ...
  resize_bilinear_gpu(d_tar, d_src, tarW, tarH, width, height, tactis);
